...
class VGG19:
    ...
    def get_deconvoluted_feat(self,feat,feat_layer_num,iters=13):
        ...
        if feat_layer_num == 5:
            ...
            noise = np.random.uniform(size=(1,512,32*scale,32*scale),low=0 , high=1) 
        elif feat_layer_num == 4:
            start_layer,end_layer = 12,20
            noise = np.random.uniform(size=(1,256,64*scale,64*scale),low=0 , high=1) 
        elif feat_layer_num == 3:
            start_layer,end_layer = 7,11
            noise = np.random.uniform(size=(1,128,128*scale,128*scale),low=0 , high=1) 
        elif feat_layer_num == 2:
            start_layer,end_layer = 2,6
            noise = np.random.uniform(size=(1,64,256*scale,256*scale),low=0 , high=1) 
        else:
